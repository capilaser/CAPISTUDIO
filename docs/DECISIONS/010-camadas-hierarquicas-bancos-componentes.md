# ADR 010 — Estrutura Hierárquica de Camadas + Bancos de Componentes
...
├── materialId (FK → materials, opcional)
...
tests/fixtures/
├── camadas-base/
│   ├── broche-simples.svg          → CAMADA_0_-_BROCHE.svg (60×25mm, path único)
│   ├── broche-completo.svg         → LOGO___NOME___TRAÇO.svg (broche montado, referência visual)
│   ├── placa-base.svg              → PLACA_2_camada_0.svg (300×90mm, retângulo arredondado)
│   └── placa-completa.svg          → PLACA_INTEIRA.svg (placa montada, referência visual)
├── apliques/
│   ├── aplique-1-formato-d.svg     → 100.2×90.2mm — formato D (retângulo + semicírculo)
│   ├── aplique-2-quadrado.svg      → 100.2×90.2mm — formato quase quadrado
│   └── aplique-3-pill.svg          → 95.2×15.2mm  — formato cápsula (pill)
...
> **Nota de coerência (pós-Onda 5):** o plano original referenciava
> `textureId → textures.id`. Esta versão usa `materialId → materials.id`
> para manter consistência com o schema vigente — a tabela `textures` foi
> renomeada para `materials` durante a Onda 5 (PNG-first strategy).
>
> Os nomes finais dos 3 apliques de fixture foram revisados para refletir
> o formato real: aplique-1-formato-d (formato D), aplique-2-quadrado
> (100×90mm, formato quase quadrado), aplique-3-pill (95×15mm, formato
> cápsula).
