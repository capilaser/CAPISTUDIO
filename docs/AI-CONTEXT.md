...
- ✅ **Refatoração modular do canvas (Onda 11 Fase B)** feita em 3 commits sequenciais com regressão zero validada por checkpoint visual + DevTools sem warnings. **Ordem invertida** (B1 toolbar → B2 workspace → B3 hook) reduziu risco — sub-fases simples primeiro servem de aquecimento antes da arriscada (hook). **Plano detalhado das 16 etapas do boot ANTES de codar** foi crítico — análise estática + paridade 1:1 conferida + bug de tipo (`CanvasMode = 'designer' | 'operator'`, eu havia escrito `'pattern' | 'operator'` no plano) pego pelo `tsc` antes do checkpoint. Padrão replicar em futuras refatorações grandes: mapear toda a ordem implícita antes de mover qualquer linha.
...
| **12**     | **Reset frontend — 3 páginas: Inicial / Arte / Banco**                          | **✅ Fechada (commits F4.1→F5, HEAD `bac9b59`)**                   |
...
_Última atualização: **Onda 12 FECHADA — Reset frontend (3 páginas)**. HEAD: `bac9b59`. Sessão 2026-05-15._

_**Inicial** (`/`, commit `b99d0e1`): lista de pedidos recentes + 2 botões grandes (Novo Pedido / Incluir Pedido em breve). **Arte** (`/arte`, commit `34f204c`): hub com 3 cartões (Novo Pedido ativo, Abrir Pedido + Criar Padrão "em breve"). **Banco** (`/banco`, stub commit `0025097`): 4 categorias (Produtos/Apliques/Gravações/Logos), 1 nível de subpasta, cadastro manual — implementação pendente (Onda futura)._

_**Novo Pedido** (`/arte/novo`, COMPLETO): F4.1 shell, F4.2 sidebar 2 estados, F4.2b topbar nome editável + cascata Categoria→Variação→Cor. **F4.3 (`23d73fc`)** canvas carrega produto + material: 3 bugs corrigidos — (a) crash React: `<canvas>` condicional conflita com Fabric (`removeChild`), fix: canvas sempre montado + overlay para erros; (b) broche sem `base_svg` no seed, fix: `BROCHE_60X25_SVG` inline igual à placa; (c) `applyMaterialToBase` usava `Group.width/height` (viewBox space, não px), fix: cria `Rect` com `clipPath` do produto (mesmo padrão de `applyMaterialToLayer`). **F5 (`bac9b59`)** menu "+ Adicionar" funcional: Colocar texto → slot no canvas + campo inline na sidebar ao vivo (múltiplos, com select de fonte); Upar SVG → file picker → parseCorelSvg → addAppliqueSvg; Usar banco de dados → BancoDrawer (520px, 4 abas: Apliques/Gravações/Marcações/Fontes reutilizando painéis existentes). `engineRef` compartilhado via prop da página._

_**Design tokens** (Onda 12.G1, commit `5235f93`): paleta violeta `#7C3AED`, fundo `#09090B`, cards `#111113`, borda `#27272A`. Badge component novo. Button variants `approve`/`svg`._

_**Lições críticas da sessão Onda 12**: (a) `<canvas>` nunca pode ser condicional com Fabric — Fabric modifica o DOM e o React perde a referência no `removeChild`. Sempre montar o canvas e usar overlay/CSS pra estados de erro. (b) `Group.width/height` em Fabric 6 retorna bounding box no espaço do viewBox (sem escala) — usar `mmToPx(productWidthMm)` como fonte autoritativa. (c) Aplicar fill em `fabric.Group` não propaga para filhos — filhos têm fill próprio. Solução correta: `Rect` separado com `clipPath` do produto._

_**Próxima onda: 13** — Multi-broche empilhado no canvas (ver `IDEAS/onda-13-multi-broche-empilhado.md`). Antes de começar: novo chat com AI-CONTEXT atualizado._
