# KICKOFF — Onda 6a — VERSÃO B

> **Use esta versão se** `ls tests/fixtures/` der erro, ou se a pasta
> existir mas estiver vazia/incompleta.
>
> **Diferença chave:** o kickoff vai criar a estrutura de fixtures e
> PARAR num checkpoint extra (Checkpoint 0), aguardando você popular
> manualmente os 7 arquivos .svg antes de continuar.
>
> **Modelo recomendado:** Sonnet 4.6 (médio)
>
> **Como usar:** anexe `AI-CONTEXT.md`, `06-VISION.md`,
> `PLANO-ADR-camadas-hierarquicas.md`, `01-DATABASE.md` E
> `README-fixtures.md` no chat do Claude Code, depois copie e cole
> TUDO entre os marcadores `═══` abaixo como primeira mensagem.
...
ETAPA PRÉVIA — CRIAR ESTRUTURA DE FIXTURES

A pasta tests/fixtures/ NÃO existe (ou está incompleta) no projeto.
Antes de qualquer migration ou seed, sua segunda tarefa é CRIAR a
estrutura vazia:
...
O README.md em tests/fixtures/ deve ser o conteúdo INTEGRAL do anexo
"README-fixtures.md".

DEPOIS DE CRIAR, PARE imediatamente. NÃO continue pra migration,
NÃO copie nada pra resources, NÃO insira no banco. Só crie a
estrutura, faça commit do ADR 010 + estrutura de fixtures, e
aguarde o usuário.

O usuário vai colocar manualmente os 7 arquivos .svg nas pastas:
...
Quando o usuário disser "fixtures populadas", você prossegue do
passo 1 (RESET LIMPO DO BANCO) em diante.
...
Leia, nesta ordem:
  ...
  4. README-fixtures.md (anexo) — conteúdo do README das fixtures.
...
ESCOPO DETALHADO DA 6a (EXECUTA APÓS O CHECKPOINT 0)
...
❌ NÃO popular as fixtures sozinho — só o usuário coloca os SVGs.
...
CHECKPOINT 0 — Estrutura de fixtures + ADR 010 criados
  Critério de saída:
    ...
    2. Pasta tests/fixtures/ existe com:
       - README.md (conteúdo do anexo README-fixtures.md)
       - camadas-base/   (pasta vazia)
       - apliques/       (pasta vazia)
       - exportacoes/    (pasta vazia)
    3. NENHUMA outra alteração no projeto além disso.
       (sem migration, sem seed, sem repository, sem schema)

  AGUARDE o usuário colocar os 7 SVGs e dizer "fixtures populadas".
  ANTES disso, NÃO toque em mais nada.

CHECKPOINT A — Schema correto
  Critério de saída:
    1. Migration nova gerada em src-tauri/migrations/.
    2. App abre sem erro de SQL.
    3. Seeds antigos (Ondas 1, 4.5, 5) rodaram automaticamente:
       - SQL: SELECT COUNT(*) FROM machines >= 3
       - SQL: SELECT COUNT(*) FROM textures >= 4
       - SQL: SELECT COUNT(*) FROM fonts >= 5
    4. Página /dev/db-check mostra:
    ...
    5. SQL: PRAGMA table_info(pattern_slots) confirma a coluna.
...
Sua primeira mensagem nesta onda deve ser EXCLUSIVAMENTE:
  ...
  1. Confirmar que leu PLANO-ADR-camadas-hierarquicas (anexo),
     AI-CONTEXT, 01-DATABASE, e README-fixtures.
  ...
  3. Apresentar o plano da 6a no formato:
     ...
     - [passos numerados, ordem de execução,
        DESTACANDO o Checkpoint 0 que pausa pro usuário]
...
Após aprovação, execute em fases. Após CADA checkpoint (0, A, B, C),
